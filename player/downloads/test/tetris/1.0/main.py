"""
Tetris Online Duel client (GUI).
遵循平台啟動參數：python main.py --player <name> --server <platform_url> --game-server <gs_url> --room <room_id>
此版本依賴 game server（server.py），請確保 manifest.server_entry 已設定。
"""
...
class GameClient:
    def __init__(self, player: str, server_host: str, server_port: int, room: str):
        ...
        tk.Button(self.root, text="Send Ready", command=self._send_ready).pack()
    ...
    def _handle(self, msg: Dict):
        ...
        if mtype == "info":
            ...
            self._log("對戰開始，伺服器維持遊戲邏輯（省略 GUI 版面）")
        elif mtype == "end":
            self._log(f"遊戲結束，結果: {msg.get('result','')}")
            messagebox.showinfo("結果", msg.get("result", ""))
            self._on_close()
        else:
            self._log(str(msg))
...
def main():
    parser = argparse.ArgumentParser(description="Tetris online duel client")
    ...
    client = GameClient(args.player, host, port, args.room)
    client.start()
