...
def wait_any_key(timeout: float) -> bool:
    """
    Return True if any key is pressed within timeout seconds.
    Windows uses msvcrt; POSIX uses cbreak mode with select.
    """
    if timeout <= 0:
        return False
    if os.name == "nt":
    ...
    try:
        ...
        # Fallback: cannot read single key without enter; don't block.
        try:
            rlist, _, _ = select.select([sys.stdin], [], [], timeout)
            if rlist:
                sys.stdin.readline()
...
def play_network(server: str, platform_server: str, room: str, player: str):
    ...
    while True:
        ...
        safe_to_exit = bool(state.get("safe_to_exit"))
        ...
        if status == "finished":
            if not exit_requested:
                if wait_any_key(0.5):
                    exit_requested = True
            if exit_requested and safe_to_exit:
                leave_room_platform(platform_server, room, player)
                return
            time.sleep(0.2)
            continue
